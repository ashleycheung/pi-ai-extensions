...
// ── Logger ─────────────────────────────────────────────────────────
// Hidden by default (including startup / health-check output).
// Set BANSOS_VERBOSE=1 to see it again.
type LogLevel = "info" | "warn" | "error" | "audit";
const VERBOSE =
  process.env.BANSOS_VERBOSE === "1" || process.env.BANSOS_VERBOSE === "true";
...
export default async function (pi: ExtensionAPI) {
  ...
  log(
    "info",
    `${aliveModels.length} model(s) registered: ${aliveModels
      .map((m) => m.id)
      .join(", ")}`
  );
